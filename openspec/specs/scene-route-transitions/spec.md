...
This capability defines how the WebGL tunnel scene responds to route changes — transitioning between an `immersive` state on the home page and a `content` state on about and work pages. It specifies the zustand-driven mode, animated alpha fades, UI chrome visibility, interruptibility, and initial-load behavior.
